...
func pos_to_grid(node:Node, pos := get_global_mouse_position(), grid_step := Vector2(32, 32)):
	node.global_position = Vector2((grid_step.x - int(pos.x) % int(grid_step.x)) + int(pos.x) , grid_step.y - (int(pos.y) % int(grid_step.y)) + pos.y)
